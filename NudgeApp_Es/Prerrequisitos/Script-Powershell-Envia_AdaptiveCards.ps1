...
$excelFilePath = "C:\Users\CopilotAdmin\Downloads\Nudge Teams App\Users.xlsx"
...
$logFolder  = "C:\Users\CopilotAdmin\Downloads\Nudge Teams App\logs\"
...
# --- Adaptive Card Setup (Interactive Picker) ---

# Folder containing your Adaptive Card JSON files
$adaptiveCardFolder = "C:\Users\CopilotAdmin\Downloads\Nudge Teams App\NudgeApp - Extended\Library\Edicion de Prompts\Copilot Basico"

if (-not (Test-Path $adaptiveCardFolder)) {
    Write-Error "Adaptive Card folder not found at: $adaptiveCardFolder"
    exit
}

# Get all JSON files in the folder (sorted by name)
$jsonFiles = Get-ChildItem -Path $adaptiveCardFolder -Filter "*.json" -File | Sort-Object Name

if (-not $jsonFiles -or $jsonFiles.Count -eq 0) {
    Write-Error "No .json files found in: $adaptiveCardFolder"
    exit
}

Write-Host "`nSelect the Adaptive Card JSON to send:`n" -ForegroundColor Cyan
...
do {
    $selection = Read-Host "`nEnter a number (1-$($jsonFiles.Count))"
} while (
    -not [int]::TryParse($selection, [ref]$null) -or
    [int]$selection -lt 1 -or
    [int]$selection -gt $jsonFiles.Count
)

$selectedFile = $jsonFiles[[int]$selection - 1]
$adaptiveCardPath = $selectedFile.FullName

Write-Host "`nUsing Adaptive Card JSON: $($selectedFile.Name)" -ForegroundColor Green
